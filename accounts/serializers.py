...
# Ce fichier est volontairement vide à ce stade, car l’application CONTRACT-IT utilise
# des vues Django classiques basées sur les templates HTML, sans API.
#
# ➤ Lorsque la version stable HTML de la plateforme sera complétée,
# la transition vers un frontend React impliquera la création d’APIs REST.
# À ce moment-là, ce fichier devra contenir des classes héritant de `serializers.ModelSerializer`
# ou `serializers.Serializer` pour convertir les objets Django en JSON et vice versa.
#
# Exemples futurs :
#     - RegisterSerializer : pour enregistrer un nouvel utilisateur via une requête API POST
#     - CustomUserSerializer : pour exposer les infos d’un utilisateur connecté via un token JWT
#     - ProjectSerializer, BidSerializer, etc. : pour représenter les objets des autres apps
#
# Ces serializers seront utilisés dans les vues API (APIView, ViewSet...) pour
# structurer les échanges entre le backend Django et le frontend React.
